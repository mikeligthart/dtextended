...
sub add_to_loop {
  ...
  if ('sequencer') {
    my $sound = q[combine(asynth(2, $n[$note % @n], $len, $spc, [3, 0.5423, 0.5952, 2*8*3, 0.285, 5, 2, (2+8), -(2/3), 3], [3, 0.8572, 0.5894, 0, 0.3908, 1*5, 7, 6, 2, 1],  mseries([3, 0.4504, 0.2655, 2*2, 0.3222, 7, 5, 1, -(1/3), 1], 1, 7, 0.7149), [2, 0.1919, 0.08502, 0, 0.1273, 8, 7, 2, -(2/2), 3],  mseries([3, 0.432, 0.277, 0, 0.06648, 5, 3, 4, -(4/4), 1], 1, 7, 0.2228), [5, 0.4803, 0.006465, 1, 0.7108, 8, 2, 6, -(1/1)*(4/2)*(1/2)*(3/2)*(3/1)*(3/4), 1/2], [2, 0.564, 0.01959, (5+6)*4, 0.1209, 6, 5, 7, -(2/4)*(3/2), 1/3],  mseries([1, 0.5682, 0.2364, 0, 0.5768, 4, 6, 3, 1, 1/3], 3, 3, 0.7726),  mseries([2, 0.7014, 0.4148, 0, 0.1832, 8, 4, 1, 3, 3], 4, 1, 0.04201), [1, 0.6945, 0.4537, 1*7, 0.5454, (7+8)*8, (1+3), 3, -(1/2), 1],  mseries([5, 0.1363, 0.07945, 7*7, 0.03764, 2, 5, 5, -(1/1), 1], 2, 9, 0.8763), [5, 0.03326, 0.9698, 6*1, 0.1678, 1, 1, 7, -(3/2)*(1/2)*(1/2)*(4/3)*(3/3), 1], [1, 0.09761, 0.7612, 4, 0.6108, 7, (7+2*3), 5, 1, 1]),backwards(reverb(lowpass(wah(asynth(1, $n[$note % @n], $len, $spc,  mseries([6, 0.6112, 0.3026, 0, 0.6518, 2, 7, 4*7, 4, 1], 2, 2, 0.03967), [1, 0.2307, 0.5086, 0, 0.9885, 2, 5, 6, 1, 1], [1, 0.612, 0.8852, 6, 0.5667, 7*4, 1, 1, 4, 1],  mseries([1, 0.1609, 0.3057, 0, 0.9482, 3, 6, 3, -(1/3), 1], 1, 2, 0.2616), [4, 0.4142, 0.5143, 4, 0.426, (2+6), 8, (2*8+5), -(3/3), 3]),0.3198*(6),0.005124*0.8581,0),$lowrad,$lowcyc),0.5,3)),0.09+0.11+0.15+0.14+0.03)];
  ...
  if ('tune') {
    ...
    my ($lowrad, $lowcyc, $hirad, $hicyc) = (((2+(4*2+(2*3+8))*3)+8),4,6,2);
    my $remain=$l->nelem/5;
my $rest;
foreach(1..3){
...
$l = offset($l,2*4*4+1*4+0);
  ...
  if ('tune') {
    my $sound = q[backwards(reverb(asynth(0.25, $n[$note % @n], $len, $spc, [5, 0.6164, 0.03424, 0, 0.8186, 8, 4, 6, 3*7, 1],  mseries([2, 0.9812, 0.6738, (1+5)*6, 0.2097, (6+3), 4, 4, -(2/1), 1], 1, 4, 0.1354), [1, 0.7228, 0.3746, 0, 0.01929, 5, 6, 2, -(2/2), 1],  mseries([5, 0.6817, 0.4324, 5*3, 0.04457, (2*(4+(2+3))+3*8*3*1)*2, 1, 3*7, 6, 1/4], 1, 5, 0.6858),  mseries([1, 0.1514, 0.08617, 0, 0.8743, 3, 2, 2, -(4/1), 1/2], 1, 8, 0.6893)),1.5,1))];
    my ($note, $param, $len) = (2,1,nlen(4*(3/4)*(1/4)));
    my ($vol, $pan, $groove, $phase) = (0.01+0.01+0.15+0.05+0.06,0.4,0,(8*(4+7*3)+7));
    my ($lowrad, $lowcyc, $hirad, $hicyc) = (4,4*7,4,5);
    my $remain=$l->nelem/5;
my $rest;
foreach(1..1){
 foreach my $ref([6, nlen(4*(2/4)), 0], [4, nlen(4*(2/3)*(3/2)), nlen(4*(1/2)*(2/4))], [7, nlen(4*(1/1)), 0]){
  ($note,$len,$rest) = @$ref;
  $l->dice(X,[0],[0],[0]) .= pdl([getsound(eval '"'.$sound.'"'),$vol,$pan,$groove,$phase]);
  $l=offset($l,-int($len+$rest));
  $remain-=$len+$rest;
